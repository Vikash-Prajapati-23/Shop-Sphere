...
export async function getSingleProduct(req, res) {
  ...
  try {
    ...
    let foundByProductId = await product.findOne({ productId: Number(id) });
    console.log(foundByProductId)
    if (foundByProductId) {
      const obj = foundByProductId.toObject();
      obj.id = obj.productId || obj._id;
      return res.status(200).json(obj);
    }
    // Try to find by productId as a string (for IDs like 'FS-20')
    // foundByProductId = await product.findOne({ productId: id });
    // if (foundByProductId) {
    //   const obj = foundByProductId.toObject();
    //   obj.id = obj.productId || obj._id;
    //   return res.status(200).json(obj);
